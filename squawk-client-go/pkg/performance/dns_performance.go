...
// NewDNSPerformanceMonitor creates a new performance monitor. A client
// certificate or CA certificate that is configured but fails to load is
// treated as a hard error rather than silently falling back to a weaker
// (or missing) mTLS/verification posture.
func NewDNSPerformanceMonitor(cfg *client.Config, log logger.Logger) (*DNSPerformanceMonitor, error) {
	...
	if cfg.ClientCert != "" && cfg.ClientKey != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to load client certificate for performance monitor: %w", err)
		}
	...
	if cfg.CaCert != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to read CA certificate for performance monitor: %w", err)
		}
		caCertPool := x509.NewCertPool()
		if !caCertPool.AppendCertsFromPEM(caCert) {
			return nil, fmt.Errorf("failed to parse CA certificate for performance monitor")
		}
