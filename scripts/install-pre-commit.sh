...
verify() {
    ...
    # Use --git-common-dir (not "$root/.git") so this resolves correctly from
    # inside a worktree too: worktrees share hooks via the main repo's hooks
    # dir, and "$root/.git" is a file (not a directory) in a worktree, which
    # silently produces false "NOT INSTALLED" reports otherwise.
    local git_common_dir
    git_common_dir="$(git rev-parse --git-common-dir 2>/dev/null)" || err "Not inside a git repository"
    [[ "$git_common_dir" = /* ]] || git_common_dir="$root/$git_common_dir"
    for hook in pre-commit pre-push; do
        target="$git_common_dir/hooks/$hook"
        if [[ ! -f "$target" ]]; then
