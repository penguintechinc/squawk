#!/usr/bin/env bash
# =============================================================================
# Squawk Alpha Deployment Script
...
#   NAMESPACE             Target namespace (default: squawk-alpha)
#   APP_HOST              Application hostname (default: squawk.localhost.local)
#
# =============================================================================
...
readonly NAMESPACE="${NAMESPACE:-squawk-alpha}"
readonly APP_HOST="${APP_HOST:-squawk.localhost.local}"
readonly OVERLAY_PATH="${OVERLAY_PATH:-k8s/kustomize/overlays/alpha}"

# Services with Dockerfiles (customize per repo)
declare -a SERVICES=("dns-webui")
...
OPTIONS:
    ...
    --service SERVICE     Build specific service only (e.g., dns-webui)
